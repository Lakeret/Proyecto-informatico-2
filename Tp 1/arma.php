...
<div class="arma-detail container">
    ...
    <div class="arma-details-panel">
        ...
        <form method="post" action="cart_action.php" class="d-inline-block mb-4">
            <input type="hidden" name="action" value="add">
            <input type="hidden" name="id" value="<?= intval($arma['id_arma']) ?>">
            <input type="hidden" name="quantity" value="1">
            <input type="hidden" name="redirect" value="<?= htmlspecialchars($_SERVER['REQUEST_URI']) ?>">
            <button type="submit" class="btn btn-warning me-3">Agregar al carrito</button>
        </form>
        <div class="mt-4 d-inline-block">
            <a href="categoria.php?categoria=<?= urlencode($arma['categoria']) ?>" class="btn btn-outline-secondary me-3">Volver a <?= $categoryName ?></a>
            <a href="categoria.php?categoria=all" class="btn btn-primary">Ver catálogo completo</a>
        </div>
    </div>
</div>
